...
int UDP_recv(SOCKET s, char *buf, int maxSize, char *remoteHost, char *remotePort) {    // Parameter List:
    // [IN] s            : SOCKET handle
    ...
    if (s < 0 || s == INVALID_SOCKET) {
        ...
        if (numBytes <= 0) {
            std::cout << std::endl << "Problem with recvfrom() function call within UDP_recv().  Error Code = "
                      << WSAGetLastError() << std::endl;
        } else if (numBytes > maxSize) {
            std::cout << std::endl
                      << "Problem with recvfrom() function call within UDP_recv().  Received data exceeded buffer size"
                      << std::endl;
        } else {
