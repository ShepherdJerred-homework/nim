...
int UDP_send(SOCKET s, char *buf, int numBytesToSend, const char *host, const char *service) {    // Parameter List:
    // [IN] s      : SOCKET handle
    ...
    if (s < 0 || s == INVALID_SOCKET) {
        ...
        if (numBytes <= 0) {
            std::cout << std::endl << "Problem with sendto() function call within UDP_send().  Error Code = "
